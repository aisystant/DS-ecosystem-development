...
> Тип: межсистемный сценарий
> Владелец: зависит от масштаба (День → Стратег, Сессия → Кодировщик)
> Источник: WP-94, Левенчук (2026-03-12)

### 3.1. Три типа Handoff

| Тип | Что передаётся | Артефакт |
|-----|----------------|----------|
| **Human→Agent** | Задание + контекст | WP context file (Сессия), DayPlan (День) |
| **Agent→Human** | Результат + что проверить | Session Close report (Сессия), Day Close итоги (День) |
| **Agent→Agent** | Контекст + промежуточный результат | WP context file «Осталось» (между сессиями), DayClose «задел на завтра» (между днями) |

### 3.2. Handoff на масштабе День

| Момент | Handoff | Артефакт | Минимум |
|--------|---------|----------|---------|
| Day Open | Стратег→Человек | DayPlan | Приоритеты, бюджет, контекст |
| Day Close | Человек→Стратег | Итоги дня (DayPlan §Итоги) | Что сделано, что узнал, что осталось |
| Day Close → Day Open | Вечерний Стратег→Утренний Стратег | «Задел на завтра» | С чего начать, какой контекст подготовить |

### 3.3. Handoff на масштабе Сессия

| Момент | Handoff | Артефакт | Минимум |
|--------|---------|----------|---------|
| Session Open | Human→Agent | WP context file | Описание, критерий готовности, verification_class |
| Session Close | Agent→Human | Close report | Что сделано, captures, что проверить, в чём не уверен |
| Между сессиями | Agent→Agent | WP context «Осталось» | Что не завершено, какие файлы затронуты, следующий шаг |

### 3.4. Agent→Agent handoff (curated review bundle)

> Паттерн: один агент собирает контекст → передаёт другому для ревью/продолжения.
> Пример (Левенчук): Codex генерирует код → ChatGPT Pro ревьюит.
> В IWE: Кодировщик (Sonnet) делает → Архитектор (Opus) ревьюит.

**Артефакт bundle:**

| Элемент | Обязательно | Описание |
|---------|-------------|----------|
| Diff / список изменений | Да | Что изменено |
| Контекст решения | Да | Почему так, альтернативы |
| Verification hints | Да | Что проверить, на что обратить внимание |
| Файлы для чтения | Нет | Если ревьюер не знает контекст |

**Coordination Cost Check (SOTA.006):** перед созданием мультиагентной цепочки → три условия: (1) context isolation, (2) parallelism gain, (3) tool specialization. Все три «нет» → single-agent.

---

## 4. Формат описания процесса (шаблон)
