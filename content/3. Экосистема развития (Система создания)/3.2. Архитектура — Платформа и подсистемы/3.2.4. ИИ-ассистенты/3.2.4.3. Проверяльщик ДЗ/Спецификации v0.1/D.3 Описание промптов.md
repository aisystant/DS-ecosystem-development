...
```javascript
{
  ...
  max_tokens: 800,
  system: "Ты — опытный наставник образовательной программы. Проверяй ответы студентов доброжелательно, конструктивно, опираясь на норматив. Отвечай строго в JSON-формате.",
  messages: [...]
}
```
...
1. **Доброжелательность** — конструктивная критика, поддержка студента
...
## Критерии оценки
- main_idea (40%): Студент передал ключевую идею
...
Студент видит Markdown-комментарий:
