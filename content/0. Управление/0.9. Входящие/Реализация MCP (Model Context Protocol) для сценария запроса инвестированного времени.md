...
↓ 
Проводник (ассистент)   
├─ читает dt://catalogs/metrics   ← что такое “инвестированное время”  
├─ выбирает tool по manifest      ← dt.get_time_invested_summary   
↓ 
MCP клиент → MCP сервер   
├─ читает dt://catalogs/metrics   ← какие типы активности учитывать  
├─ читает dt://policies/data_quality ← как валидировать результат 
↓ 
Digital Twin Database  
└─ выбирает данные activity_log   ← по фильтрам из resources  
↑ 
MCP сервер возвращает результат 
↑ Проводник читает dt://catalogs/levels ← соотносит время с уровнем ученика   
↑ Человек получает ответ с пояснением`
