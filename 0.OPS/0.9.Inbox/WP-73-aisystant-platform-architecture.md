...
- **11 сценариев (SC-1 → SC-11):** онбординг, обучение, целевые системы, ЦД-персонализация, токены и репутация, сообщество, мультиповерхностный доступ, командообразование, масштабирование (Global Core + Local Edge), управление потоками и наставничество, назначение на позицию
...
| 11 | Proof-of-Impact (токены) | ❌ PLANNED | Токеновая экономика |
...
| **Токены и репутация** | Один токеновый движок, один эпистемический статус |
...
```
    ...
    |                                                     |
    |  Community . Tokens . DT . Knowledge . AI systems   |
...
| 13 | **Loyalty / Proof-of-Impact** | ❌ Не существует | Токеновый движок | 🔴 Создать | **P2** |
...
|---------|--------|-----------|
| 🔴 **Создать уникальное** | 8 | CRM/Бэк-офис, Loyalty/Proof-of-Impact, Activity Hub, Revenue Sharing (в Billing), Панель наставника/Потоки, Personal Guide Gen, ESG, Team Service |
...
| Г | **Генеративность** | **9** | Работает в шаблоне экзокортекса. Создаёт платформу. Apps SDK + маркетплейс. Global Core + Local Edge. Двойник + ESG + токены — замкнутый цикл роста |
...
| # | Вопрос |
|---|--------|
| Q10 | Единый тариф в разных валютах или региональные цены? |
| Q12 | Выбор РФ-хостинга (Hetzner Russia vs VK Cloud vs Selectel) |
...
- [x] 60 сервисов (43 + 17 новых S44–S60)
...
**Цель:** Токены + ESG + команды + сертификация
...
**Milestone:** Полная экосистема: обучение + токены + ESG + сертификация + команды + мониторинг.
