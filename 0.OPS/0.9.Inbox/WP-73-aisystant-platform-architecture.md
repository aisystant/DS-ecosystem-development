...
- [x] To-be архитектура v2 (16 принципов P1–P16, ASCII-диаграмма, 22 ключевых изменения)
- [x] ЭМОГССБ-оценка: **8.3/10** (проходит АрхГейт)
- [x] Gap-анализ: **31 подсистема** (8 создать уникальное, 5 интегрировать SOTA, 11 доработать, 7 минимальный)
...
- [x] 10 сценариев использования (SC-1 → SC-10, вынесены в WP-74)
...
- [x] Роли: учащиеся, наставники, операторы (бухгалтер/CRM, админы) — вынесены в WP-74
- [x] Панель наставника, CRM-панель, панель бэк-офиса — описаны в WP-74 §2
...
#### Тиры обслуживания (DP.ARCH.002 → детали в [WP-74 §3](WP-74-platform-concept-of-use.md))
...
> - 3-й уровень (проектные) → ADR в этом документе (ниже)
> - 1-й уровень (FPF) → ссылки, не дублируем

#### Принципы 2-го уровня (из DP.ARCH.001 §7, source-of-truth)
...
#### Принципы 3-го уровня (проектные решения Aisystant)
...
| 4 | **Billing + Revenue Sharing** | 🟡 TG Stars в боте, LMS programs на system-school.ru | **Приём оплаты:** Stripe (мир) + YooKassa (РФ) + TG Stars — готовые SOTA-модули, интеграция через webhook. **Revenue Sharing (уникальное):** автоматическое разделение каждого платежа: платформа 30% / автор 50% / наставник 15% / куратор 5%. Начисления наставникам, инвестиции, операционка. **Unit economics:** MRR, ARR, LTV, CAC, churn по когортам и программам. **Feature gating:** tier из JWT (ADR-014) | 🟡 Интеграция SOTA (Stripe/YooKassa) + 🔴 Создать Revenue Sharing движок и unit economics dashboard | **P1** |
...
| 12 | **CRM / Бэк-офис (T0)** | ❌ Не существует | **Путь клиента (customer journey):** полная история от первого касания до текущего статуса — все оплаты, программы, потоки, сертификаты. **Воронка:** лид → регистрация → trial → подписка → retention. **Бэк-офис:** панель для бухгалтера (оплаты, начисления наставникам, Revenue Sharing отчётность), администратора (потоки, участники), маркетолога (CAC, конверсии). **Метрики:** MRR/churn/LTV по когортам | 🔴 Создать (уникальная бизнес-логика поверх данных Neon; внешняя CRM не знает наших тиров, ролей и ЦД) | **P2** |
...
| 16 | **Team Service** | ❌ Не существует | Формирование команд, канальная интеграция (TG-группы / Web App), ИИ-агенты | 🔴 Создать | **P3** |
| 17 | **Реал-тайм каналы команд** | 🟡 Telegram уже используется | SC-8: TG-группы (уже есть аудитория) или Web App чат (Фаза 2-3). Discord — опция, но не приоритет (P16: не вводить новую платформу без необходимости) | 🟡 Интеграция TG-групп | **P3** |
...
| 30 | **Revenue Sharing** | ❌ Не существует (формула описана: platform 30%, author 50%, instructor 15%, curator 5%) | Автоматическое распределение доходов по формуле. **Объединён с Billing (#4)** как единый движок | → #4 | **P1** |
...
### Сводка Gap (с учётом P16: Integrate SOTA, don't reinvent)

| Тип gap | Кол-во | Подсистемы |
|---------|--------|-----------|
| 🔴 **Создать уникальное** (нет на рынке) | 8 | CRM/Бэк-офис (уникальная модель: тиры + ЦД + Revenue Sharing), Loyalty/Proof-of-Impact, Activity Hub, Revenue Sharing движок (в составе Billing), Панель наставника/Потоки, Personal Guide Gen, ESG, Team Service |
| 🔵 **Интегрировать SOTA** (готовые модули) | 5 | Web App (Next.js — фреймворк SOTA), Event Bus (Outbox + pg_notify — паттерн SOTA), Notification Svc (Novu/Courier — open-source), Unified Search (pgvector + FTS — SOTA в Neon), Apps SDK |
| 🟡 **Доработать существующее** | 11 | ORY SSO, ЦД, Guides MCP, LMS API, Docs, ИИ-агенты, Monitoring (Grafana), ДЗ-чекер, Certification, Рабочая тетрадь, Семинары |
| 🟢 **Минимальный gap** | 7 | Knowledge MCP, Composer MCP, TG Bot, Claude Code, Discourse, SystemsSchool Bot, Реал-тайм каналы (TG) |

> **Принцип (P16):** «Создать» = только уникальная бизнес-логика. Payments (Stripe/YooKassa), Auth (ORY), Search (pgvector), Monitoring (Grafana), Notifications (Novu) — интегрируем готовое.
...
> АрхГейт (DP.M.005): порог ≥8. Оценка общей архитектуры платформы, не отдельных компонентов.
> Характеристики: DP.ARCH.001 §4. Принципы (2-й уровень): DP.ARCH.001 §7. Покрытие: §7.1. Обратная связь: DP.M.005 §8.

| # | Характеристика | Оценка | Обоснование |
|---|---------------|--------|-------------|
| Э | **Эволюционируемость** | **9** | 10 принципов 2-го уровня (DP.ARCH.001 §7.1), модульность (30 подсистем → 57 сервисов), Event Bus для loose coupling, Strategy pattern для billing/PII, тиры T1→T5. Добавление нового сервиса не ломает существующие. Двух-юрисдикционная архитектура через адаптеры, не через fork кода. **Риск:** 30 подсистем — много зависимостей; без Event Bus первые фазы будут tight-coupled |
...
- **Скорость = 7** (слабое покрытие: только #19, #20 в DP.ARCH.001 §7.1) → рекомендация: добавить принцип «Latency budget per layer» (2-й уровень)
- **Обучаемость = 8** (слабое покрытие: только #4 IPO) → рекомендация: добавить принцип о capture-to-pack / feedback loop обучения платформы
...
| ADR-011 | **TG-группы для командной работы** (пересмотрен: Discord → TG) | 🔶 Предлагается | Telegram уже используется аудиторией (P16: не вводить новую платформу). Discord — опция на будущее | Q6 |
