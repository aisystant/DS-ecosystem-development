...
| P14 | **Open-source first** | Только открытый код для инфраструктурных компонентов. Проприетарное ПО — исключение с обоснованием. Снижает vendor lock-in, позволяет self-hosted |
| P15 | **Cloud-native by default** | Облачные решения по умолчанию. Self-hosted — когда требует закон (152-ФЗ) или экономика. Минимум собственных серверов |
| P16 | **Integrate SOTA, don't reinvent** | Готовые SOTA-модули для инфраструктуры (auth, payments, search, monitoring). Своё — только уникальная бизнес-логика, которой нет на рынке (ЦД, Proof-of-Impact, ИИ-агенты, Revenue Sharing) |
