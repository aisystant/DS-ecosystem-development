...
sequenceDiagram
    ...
    participant П as 🤖 Проводник<br/>(Ассистент)
    participant MCP as ⚙️ MCP-сервер
    participant БД as 🗄️ Digital Twin<br/>Database

    Note over Ч,БД: 📥 ЗАПРОС

    Ч->>П: "Сколько времени я инвестировал<br/>за неделю?"

    rect rgb(230, 245, 255)
        Note right of П: 📚 Читает ресурсы
        П-->>П: dt://catalogs/metrics<br/>→ что такое "инвестированное время"
        П-->>П: mcp-manifest.json<br/>→ выбирает tool
    end

    П->>MCP: dt.get_time_invested_summary<br/>{ period: "week" }

    rect rgb(255, 245, 230)
        Note right of MCP: 📚 Читает ресурсы
        MCP-->>MCP: dt://catalogs/metrics<br/>→ какие типы активности учитывать
        MCP-->>MCP: dt://policies/data_quality<br/>→ как валидировать результат
    end

    MCP->>БД: SELECT * FROM activity_log<br/>WHERE date >= ...
    БД-->>MCP: Сырые данные активности

    Note over Ч,БД: 📤 ОТВЕТ

    MCP-->>П: { total_hours: 9.8,<br/>average_per_day: 1.4, ... }

    rect rgb(230, 255, 230)
        Note right of П: 📚 Обогащает ответ
        П-->>П: dt://catalogs/levels<br/>→ соотносит время с уровнем ученика
    end

    П-->>Ч: "За неделю вы инвестировали<br/>9ч 48мин — по 1ч 24мин в день"
```
